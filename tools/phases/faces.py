...
The old face is erased by refilling it from the ball around it (normalized convolution), then a new face is drawn:
  phase 2  wide creepy grin full of teeth, black eyes with dark bags
  phase 3  worn out: half-closed eyes with a white glint, heavy bags, flat wavy mouth
...
def erase_face(img, cx, cy, R):
    ...
    return Image.fromarray(res, "RGBA"), lum, base
...
def face_layer(size, cx, cy, R, phase, dark, base):
    """Transparent layer with the new face, supersampled."""
    S = size * SS
    ...
    X = lambda u: (cx + u * R) * k
    Y = lambda v: (cy + v * R) * k
    ink = (14, 10, 12, 255)  # recoloured per pixel afterwards (light on dark parts of the ball)
    # dark bags under / around the eyes, soft
    bags = Image.new("RGBA", (S, S), (0, 0, 0, 0))
    bd = ImageDraw.Draw(bags)
    bag_col = (150, 70, 190, 150) if dark else (int(base[0] * 0.45 + 40), int(base[1] * 0.25), int(base[2] * 0.25 + 10), 150)
    for sx in (-1, 1):
        if phase == 2:
            bd.ellipse((X(sx * 0.30 - 0.20), Y(-0.30), X(sx * 0.30 + 0.20), Y(0.12)), fill=bag_col)
        else:
            bd.ellipse((X(sx * 0.31 - 0.22), Y(-0.08), X(sx * 0.31 + 0.22), Y(0.16)), fill=bag_col)
    bags = bags.filter(ImageFilter.GaussianBlur(0.07 * R * k))
    L.alpha_composite(bags)
    d = ImageDraw.Draw(L)
    if phase == 2:
        # eyes: tall black ovals
        for sx in (-1, 1):
            d.ellipse((X(sx * 0.30 - 0.085), Y(-0.24), X(sx * 0.30 + 0.085), Y(0.03)), fill=ink)
        # grin: crescent whose corners curl up past the eyes' width
        W = 0.66
        ...
        outer = [(X(u), Y(v - o)) for u, v in top] + [(X(u), Y(v + o)) for u, v in bot]
        ...
        for row in (0, 1):
            for i in range(n):
                ...
                u1 = -W + 2 * W * (i + 1) / n - gap
                if row == 0:
                    a, b = up, lambda u: mid(u) - 0.012
                else:
                    a, b = lambda u: mid(u) + 0.012, lo
        ...
        for sx in (-1, 1):
            u, v = sx * W, up(W)
            for j, (du, dv) in enumerate(((0.07, -0.13), (0.11, -0.05), (0.03, -0.16))):
                d.line((X(u - sx * 0.02), Y(v + 0.02), X(u + sx * du), Y(v + dv)), fill=ink, width=int(0.035 * R * k))
    else:
        # half-closed eyes: dark almond, heavy flat lid, white glint
        for sx in (-1, 1):
            ex = sx * 0.31
            lid = lambda u: -0.02 + 0.03 * ((u - ex) / 0.17) ** 2 * 0 + sx * 0.02 * (u - ex) / 0.17
            low = lambda u: 0.07 - 0.075 * ((u - ex) / 0.17) ** 2
            pts = curve(ex - 0.17, ex + 0.17, lid) + curve(ex + 0.17, ex - 0.17, low)
            d.polygon([(X(u), Y(v)) for u, v in pts], fill=ink)
            # droopy lid line, a bit wider than the eye
            d.line([(X(u), Y(lid(u) - 0.01)) for u, v in curve(ex - 0.20, ex + 0.20, lid, 12)], fill=ink, width=int(0.05 * R * k), joint="curve")
            glint = (255, 255, 255, 255)
            d.ellipse((X(ex - 0.03), Y(0.005), X(ex + 0.03), Y(0.065)), fill=glint)
        # flat wavy mouth
...
def make(slug):
    ...
    clean, lum, base = erase_face(img, cx, cy, R)
    dark = lum < 70
    outs = {}
    for phase in (2, 3):
        o = clean.copy()
        layer = np.asarray(face_layer(size, cx, cy, R, phase, dark, base)).astype(np.float32)
        ...
        rgb[ink] = (238, 234, 250)
        rgb[glint] = (20, 14, 26)
        o.alpha_composite(Image.fromarray(np.dstack([rgb, layer[..., 3]]).clip(0, 255).astype(np.uint8), "RGBA"))
        outs[phase] = o
