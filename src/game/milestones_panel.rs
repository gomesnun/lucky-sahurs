...
use super::base::Bo;
use super::{Game, cb};
use crate::core::data::{MILESTONES, milestone_cat, milestone_reward_label};
...
impl Game {
    ...
    pub fn draw_milestones_panel(&mut self, rect: Rect, mouse_pos: (f64, f64)) {
        ...
        if let Some(cat) = category {
            ...
            self.button(back_rect, &tr("<  Categories"), &sb, mouse_pos, panel_light(), panel(), WHITE, cb(|g| g.select_milestone_category(None)), Bo::r(8));
            let claimed_n = self.claimed_count(cat, d.tiers.len());
            let cat_txt = self.f.med.render(&format!("{}  ({}/{})", tr(d.label), claimed_n, d.tiers.len()), accent());
        ...
        let content_h = match category {
            None => self.draw_milestones_category_list(content_rect, scroll, mouse_pos),
    ...
    fn draw_milestones_category_list(&mut self, content_rect: Rect, scroll: f64, mouse_pos: (f64, f64)) -> f64 {
        let pad = 20;
        ...
        for d in MILESTONES.iter() {
            ...
            if row_rect.bottom() > content_rect.top() && row_rect.top() < content_rect.bottom() {
                ...
                let name_txt = med.render(&tr(d.label), WHITE);
