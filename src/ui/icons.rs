...
thread_local! {
    ...
    static PETS: RefCell<HashMap<(String, i32, bool), Option<Surf>>> = RefCell::new(HashMap::new());
}
...
pub fn load_pet_image(pet_name: &str, size: i32, silhouette: bool) -> Option<Surf> {
    let key = (pet_name.to_string(), size, silhouette);
    if let Some(v) = PETS.with(|c| c.borrow().get(&key).cloned()) {
        return v;
    }
    let slug = pet_slug(pet_name);
    let surf = crate::assets::read(&format!("icons/pets/{}.png", slug)).and_then(|b| load_png_bytes(&b)).map(|img| {
